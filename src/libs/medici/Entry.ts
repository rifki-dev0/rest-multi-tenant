...
import { InvalidAccountPathLengthError, TransactionError } from "./errors";
...
import { TJournalDocument } from "@/libs/medici/models/journal";
...
> {
  ...
  static write<U extends ITransaction, J extends IJournal>(
    ...
    return new this(book, memo, connection, date, original_journal);
  }
  ...
  constructor(
    ...
    this.journal = new connection.journal() as TJournalDocument<J> & {
    ...
    try {
      await Promise.all(
        this.transactions.map((tx) =>
          new this.connection.transaction(tx).validate(),
        ),
      );
      await this.journal.validate();

      const result = await this.connection.transaction.collection.insertMany(
        this.transactions,
        {
      ...
      if (!insertedIds[0]) {
        ...
        const txs = await this.connection.transaction.collection
          .find(
      ...
      await this.connection.journal.collection.insertOne(
        this.journal.toObject(),
        options,
      );
