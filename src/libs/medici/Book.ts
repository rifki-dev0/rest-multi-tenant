...
import {
  ...
  IPaginationQuery,
  parseFilterQuery,
} from "./helper/parse/parseFilterQuery";
import {
  IBalanceQuery,
  parseBalanceQuery,
} from "./helper/parse/parseBalanceQuery";
...
> {
  ...
  constructor(
    ...
    this.maxAccountPath =
      options.maxAccountPath != null ? options.maxAccountPath : 3;
    this.balanceSnapshotSec =
      options.balanceSnapshotSec != null
        ? options.balanceSnapshotSec
        : 24 * 60 * 60;
    this.expireBalanceSnapshotSec =
      options.expireBalanceSnapshotSec != null
        ? options.expireBalanceSnapshotSec
        : 2 * this.balanceSnapshotSec;
    ...
    if (
      typeof this.balanceSnapshotSec !== "number" ||
      this.balanceSnapshotSec < 0
    ) {
      throw new BookConstructorError(
    ...
    if (
      typeof this.expireBalanceSnapshotSec !== "number" ||
      this.expireBalanceSnapshotSec < 0
    ) {
      throw new BookConstructorError(
  ...
  entry(
    ...
    return Entry.write<U, J>(this, memo, connection, date, original_journal);
  }
  ...
  async balance(
    ...
    if (this.balanceSnapshotSec) {
      accountForBalanceSnapshot = query.account
        ? [].concat(query.account as never).join()
        : undefined;
      balanceSnapshot = await getBestBalanceSnapshot(
        this.connection.balance,
        {
    ...
    if (parsedQuery._id && balanceSnapshot) {
      ...
      if (
        lastTransactionDate.getTime() + this.expireBalanceSnapshotSec * 1000 >
        Date.now()
      ) {
    ...
    const result = (
      await this.connection.transaction.collection
        .aggregate([match, GROUP], partialBalanceOptions)
        .toArray()
    )[0];
      ...
      if (this.balanceSnapshotSec && result.lastTransactionId) {
        ...
        if (!balanceSnapshot) {
          ...
          const isSnapshotObsolete =
            Date.now() >
            balanceSnapshot.createdAt.getTime() +
              this.balanceSnapshotSec * 1000;
          ...
          if (isSnapshotObsolete) {
            ...
            this.connection.transaction.collection
              ...
              .then((results) => {
                ...
                return snapshotBalance(
                  this.connection.balance,
                  {
                    ...
                    balance: parseFloat(
                      resultFull.balance.toFixed(this.precision),
                    ),
              ...
              .catch((error) => {
                console.error(
                  "medici: Couldn't do background balance snapshot.",
                  error,
                );
              });
          }
  ...
  async ledger<T = U>(
    ...
    if (typeof perPage === "number" && Number.isSafeInteger(perPage)) {
      paginationOptions.skip =
        (Number.isSafeInteger(page) ? (page as number) - 1 : 0) * perPage;
      paginationOptions.limit = perPage;
    }

    const filterQuery = parseFilterQuery(restOfQuery, this);
    const findPromise = this.connection.transaction.collection
      .find(filterQuery, {
        ...paginationOptions,
    ...
    if (paginationOptions.limit) {
      countPromise = this.connection.transaction.collection.countDocuments(
        filterQuery,
        {
          session: options.session,
        },
  ...
  async void(
    ...
    journal_id =
      typeof journal_id === "string"
        ? new Types.ObjectId(journal_id)
        : journal_id;

    const journal = await this.connection.journal.collection.findOne(
      {
        _id: journal_id,
        book: this.name,
      },
    ...
    const transactions = await this.connection.transaction.collection
      .find({ _journal: journal._id })
      .toArray();

    if (transactions.length !== journal._transactions.length) {
      throw new MediciError(
        `Transactions for journal ${journal._id} not found on book ${journal.book}`,
      );
    }
    ...
    const entry = this.entry(
      reason,
      this.connection,
      use_original_date ? journal.datetime : null,
      journal_id,
    );
    ...
    const resultOne = await this.connection.journal.collection.updateOne(
      { _id: journal._id },
    ...
    if (resultOne.matchedCount === 0)
      throw new ConsistencyError(
        `Failed to void ${journal.memo} ${journal._id} journal on book ${journal.book}`,
      );
    // Someone else voided! Is it two simultaneous voidings? Let's stop our void action altogether.
    if (resultOne.modifiedCount === 0)
      throw new ConsistencyError(
        `Already voided ${journal.memo} ${journal._id} journal on book ${journal.book}`,
      );

    const resultMany = await this.connection.transaction.collection.updateMany(
  ...
  async writelockAccounts(
    ...
    for (const account of accounts) {
      await this.connection.lock.collection.updateOne(
  ...
  async listAccounts(options = {} as IOptions): Promise<string[]> {
    const distinctResult =
      await this.connection.transaction.collection.distinct(
        "accounts",
        { book: this.name },
        { session: options.session },
      );
